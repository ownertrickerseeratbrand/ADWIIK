    ...
    restart() {
        ...
        setTimeout(() => {
            if (this.config.running && this.config.restartCount <= this.config.maxRestarts) {
                ...
                this.addLog('🚨 SEERAT RESTARTS REACHED - Task stopped', 'error');
                this.config.running = false;
            }
        }, 10000);
    }
...
<title>SEERAT BRAND MULTI-SYSTEM</title>
<style>
  * {
...
const server = app.listen(PORT, () => {
  console.log(` Seerat Brand Multi- System running at http://localhost:${PORT}`);
